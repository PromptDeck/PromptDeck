...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const templates = {
    ...
    project_vendor_mail: {
      customPrompt: `
請協助我為{{audience}}設計一封具高度專業感的邀約信，主題「{{topic}}」，將於{{platform}}平台發送。請依以下結構撰寫：
...
`
    ...
    project_word_report: {
      customPrompt: `
請以{{tone}}語氣，為{{audience}}撰寫「{{topic}}」完整專案企劃書，並於{{platform}}使用。請分段呈現以下內容：
...
`
    ...
    project_ppt: {
      customPrompt: `
請協助設計一份{{audience}}用於{{platform}}的專案簡報（{{topic}}），並深度細化每個分頁內容，結構如下：
...
`
    ...
    project_name: {
      customPrompt: `
請協助設計3~5組適合{{audience}}的專案名稱（{{topic}}），須兼顧創意、專業與傳播力，結構如下：
...
`
    ...
    meeting_name: {
      customPrompt: `
請根據{{topic}}及{{audience}}，設計3~5個會議名稱，讓名稱能清楚反映主題、氣氛與目標。結構如下：
...
`
    ...
    satisfaction_survey: {
      customPrompt: `
請為{{audience}}設計一份「{{topic}}」滿意度問卷，需符合以下結構與邏輯：
...
`
    ...
    exchange_survey: {
      customPrompt: `
請設計一份專為{{audience}}（如團隊成員、合作夥伴）進行經驗交流的問卷，主題為「{{topic}}」，總題數5~7題，並依以下結構規劃：
...
// ...前段已如上一則（專案類、exchange_survey已寫完）

    // --- 商務應用 ---
    b2b_intro_mail: {
      customPrompt: `
請以{{tone}}語氣，協助我設計一封針對{{audience}}的B2B業務開發郵件，主題為「{{topic}}」，目標{{goal}}，結構如下：
...
`
    ...
    annual_report: {
      customPrompt: `
請為{{audience}}撰寫「{{topic}}」年度業績簡報，適用於{{platform}}簡報會議，並細化每分頁內容：
...
`
    ...
    business_reply: {
      customPrompt: `
請協助我以{{tone}}撰寫一封專業親切的商務合作回信，針對{{audience}}就「{{topic}}」的詢問/邀約進行回覆，並依以下結構：
...
`
    ...
    cover_letter: {
      customPrompt: `
請依{{reference}}及{{audience}}，為{{userRole}}產生一封專業且具個人特色的求職信（{{topic}}），分段結構如下：
...
`
    ...
    edm_email: {
      customPrompt: `
請根據{{goal}}與{{audience}}，以{{tone}}語氣設計一封{{topic}}行銷EDM，結構如下：
...
`
    ...
    meeting_summary: {
      customPrompt: `
請協助我為{{audience}}於{{platform}}整理一份「{{topic}}」會議紀要，結構如下：
...
`
    ...
    lesson_plan: {
      customPrompt: `
請為{{audience}}設計一份結構完整、易懂且具啟發性的教學腳本，主題為「{{topic}}」，由{{userRole}}主講，適用於{{platform}}平台。請依以下結構撰寫：
...
`
    ...
    study_notes: {
      customPrompt: `
請以{{tone}}語氣協助我針對{{topic}}進行學習筆記整理，對象為{{audience}}，適用於{{platform}}。內容請結構如下：
...
`
    ...
    resume_bio: {
      customPrompt: `
請根據{{reference}}，以{{tone}}語氣協助{{userRole}}產生一份針對{{audience}}的履歷自傳，主題為「{{topic}}」，用於{{platform}}。內容請分段敘述：
...
`
    ...
    // --- 創意／行銷 ---
    // creative_copy 已於上一則補上
    ad_headline: {
      customPrompt: `
請以{{tone}}語氣，為{{audience}}針對{{topic}}激發5組廣告標題，每則控制於{{constraint}}（如10字內），可嘗試中英文、押韻、雙關語或動詞開頭。每則下方可附一句創作思路或設計亮點說明。標題風格需明確適合{{platform}}及指定受眾，參考資料如{{reference}}可輔助靈感，輸出格式為{{format}}條列。
`
    },
    event_invite: {
      customPrompt: `
請以{{tone}}語氣，為{{audience}}撰寫一則活動邀請文，主題為「{{topic}}」，內容結構如下：
...
`
    ...
    youtube_script: {
      customPrompt: `
請以{{tone}}語氣，為{{platform}}頻道的{{audience}}，主題「{{topic}}」撰寫一份完整YouTube影片腳本。腳本結構如下：
...
`
    ...
    social_post: {
      customPrompt: `
請根據{{goal}}與{{topic}}，以{{tone}}語氣為{{audience}}設計一則社群貼文，適用於{{platform}}。內容須結合品牌個性或時事梗，建議結構如下：
...
`
    ...
    newsletter: {
      customPrompt: `
請撰寫一段電子報開頭文，主題為「{{topic}}」，語氣{{tone}}，受眾為{{audience}}，適用於{{platform}}平台。內容請依以下結構：
...
`
    ...
    faq_support: {
      customPrompt: `
請為{{platform}}的{{audience}}設計一組FAQ自動回覆。內容結構如下：
...
`
    ...
    product_review: {
      customPrompt: `
請協助以{{tone}}語氣，為{{audience}}撰寫一篇{{topic}}產品開箱心得，結構如下：
...
`
    ...
    product_faq: {
      customPrompt: `
請根據{{reference}}（如產品說明書、常見問題紀錄），以{{tone}}語氣協助為{{audience}}自動生成5-7題{{topic}}FAQ。內容結構如下：
...
`
    ...
    seo_title: {
      customPrompt: `
請以{{tone}}語氣，為{{platform}}上的{{audience}}針對{{topic}}產生SEO文章標題與關鍵字，內容分為：
...
`
    ...
    midjourney_art: {
      customPrompt: `
請以{{tone}}語氣，根據{{topic}}，為{{audience}}設計適用於{{platform}}的Midjourney圖像生成Prompt，內容如下：
