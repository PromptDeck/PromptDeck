...
promptForm.addEventListener('submit', async function(e) {
  ...
  const reference = promptForm.reference.value.trim(); // 新增這行
...
document.getElementById('template-select').addEventListener('change', function() {
  ...
  if (t) {
    ...
    document.getElementById('goal').value = recommendGoal(t.group);
  }
});
function recommendGoal(group) {
  if (!group) return "";
  if (group.includes("行銷")) return "品牌塑造";
  if (group.includes("學習")) return "學習成長";
  if (group.includes("社群")) return "趣味互動";
  if (group.includes("科技")) return "效率工作";
  if (group.includes("活動")) return "品牌塑造";
  if (group.includes("客服")) return "效率工作";
  if (group.includes("職涯")) return "學習成長";
  if (group.includes("廣告")) return "品牌塑造";
  if (group.includes("圖像")) return "激發創意";
  if (group.includes("內容")) return "激發創意";
  return "";
}
